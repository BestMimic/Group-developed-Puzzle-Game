...
void CObject::CollisionResponse(const Vector2& norm, float d, CObject* pObj){
  ...
  else if(!m_bStatic && !bStatic) //both objects are dynamic
...
const Vector2 CObject::GetViewVector() const{
  return AngleToVector(m_fRoll+XM_PIDIV2); // had to add XM_PIDIV2 to m_fRoll. This rotated it by 90 degrees
} //ViewVector
...
const bool CObject::isPulse() const {
    return m_isPulse;
}

const bool CObject::isKnight() const {
    return m_isKnight;
}
...
const bool CObject::isSwitch() const {
    return m_isSwitch;
}
