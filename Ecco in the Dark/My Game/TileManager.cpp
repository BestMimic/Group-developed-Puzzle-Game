...
void CTileManager::LoadMap(char* filename){
  ...
  for(size_t i=0; i<m_nHeight; i++){
    for(size_t j=0; j<m_nWidth; j++){
      ...
      // this is for placing Silent Knights
      else if (c == 'K') {
          m_chMap[i][j] = 'F'; //floor tile
          const Vector2 pos = m_fTileSize * Vector2(j + 0.5f, m_nHeight - i - 0.5f);
          m_vecKights.push_back(pos);
      } //else if
      
      // this is for placing the Door tiles
      else if (c == 'D') {
          m_chMap[i][j] = 'F'; //floor tile
          const Vector2 pos = m_fTileSize * Vector2(j + 0.5f, m_nHeight - i - 0.5f);
          m_vecDoors.push_back(pos);
      } //else if

      else if (c == 'C') {
          m_chMap[i][j] = 'F'; //floor tile
          const Vector2 pos = m_fTileSize * Vector2(j + 0.5f, m_nHeight - i - 0.5f);
          m_vecSwitches.push_back(pos);
      }
      
      // this is for placing Buttons
      else if (c == 'B') {
          m_chMap[i][j] = 'F'; //floor tile
          const Vector2 pos = m_fTileSize * Vector2(j + 0.5f, m_nHeight - i - 0.5f);
          m_vecButtons.push_back(pos);
      }
...
void CTileManager::GetObjects(std::vector<Vector2>& turrets, Vector2& player, std::vector<Vector2>& knights, std::vector<Vector2>& doors, std::vector<Vector2>& arrow, std::vector<Vector2>& switches, std::vector<Vector2>& buttons){
  turrets = m_vecTurrets;
  player = m_vPlayer;
  knights = m_vecKights;
  doors = m_vecDoors;
  arrow = m_vecArrow;
  switches = m_vecSwitches;
  buttons = m_vecButtons;
} //GetObjects
...
/// \param t Sprite type for a 4-frame sprite: 0 is floor, 1 is wall, 2 is an error tile, 3 is when the tile is invisible.
    ...
    for(int j=left; j<=right; j++){ //for each row
      ...
      switch(m_chMap[i][j]){ //select which frame of the tile sprite is to be drawn
        ...
        case 'W':
            ...
            if (visibility)
            {
                desc.m_f4Tint = XMFLOAT4(rgb, rgb, rgb, 0);
                if (rgb >= 1.0f)
                    decreasing = true;
            }
            else 
                desc.m_f4Tint = XMFLOAT4(0.0f, 0.0f, 0.0f, 0);
            break; //wall
...
void CTileManager::ReactToPulse() { turnVisible = true; }
void CTileManager::CheckForVisibility() 
{ 
    if (turnVisible) //Checker to see if a pulse has collided with the wall
        visibility = true;
    else
        return;
    if(!decreasing) // When we want the Walls to appear
        rgb += 0.25f * m_pTimer->GetFrameTime();
    else // When we want them to disapppear
    {
        rgb -= 0.25f * m_pTimer->GetFrameTime();
        if (rgb <= 0.0f) // After the decrease has finish, reset to prepare for next pulse
        {
            rgb = 0.0f;
            decreasing = false;
            turnVisible = false;
